...
void handleRoot() {
  ...
  s=s+"<h1>캔들비랩스</h1>";
  /*
  //s=s+"먼지센서"+"&nbsp&nbsp"+dustValue+"<br>";
  if(Auto==1)
    s=s+"<a href=\"auto\"><button style='background-color:Lime; color:blue;'>Auto on</button></a>";
  else 
    s=s+"<a href=\"auto\"><button style='background-color:DarkGreen; color:white;'>Auto off</button></a>";

  //s=s+"<form action='/dustset'>먼지센서(1~60)<input type=\'number' id='dust' name='dust' min='1' max='60' value='"+setDustValue+"'><input type='submit'></form>";

  s=s+"<br><a href=\"on\"><button style='background-color:Lime; color:blue;'>켜짐 on</button></a>";
  s=s+"&nbsp&nbsp";
